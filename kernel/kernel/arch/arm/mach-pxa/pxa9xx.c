/*
 * linux/arch/arm/mach-pxa/pxa9xx.c
 *
 * Code specific to PXA9xx
 *
...
static DEFINE_PXA3_CKEN(pxa9xx_u2o, USBH_EHCI, 480000000, 0);

static struct clk_lookup pxa9xx_clkregs[] = {
	INIT_CLKREG(&clk_pxa9xx_u2o, NULL, "USBCLK"),
};

int __init pxa_usb_phy_init(unsigned int base)
{
	...
	/* check whether U2PLL[READY] is set */
	count = 100000;
	while (((u2o_get(base, U2PPLL) & U2PPLL_READY) == 0) && count--);
	if (count <= 0)
		pr_warning("%s TIMEOUT for U2PPLL[READY]\n", __func__);
	...
	if (machine_is_saar())
		u2o_set(base, U2PPLL, (0x7<<U2PPLL_ICP_SHIFT) | (0x7<<U2PPLL_KVCO_SHIFT) \
			| U2PPLL_KVCO_EXT/* | U2PPLL_CLK_BLK_EN*/);
	else
		u2o_set(base, U2PPLL, (0x6<<U2PPLL_ICP_SHIFT) | (0x4<<U2PPLL_KVCO_SHIFT) \
			| U2PPLL_KVCO_EXT | U2PPLL_CLK_BLK_EN);
	 ...
	 */
	if (machine_is_saar()) {
		u2o_clear(base, U2PRX, U2PRX_SQ_THRESH_MASK);
...
static int __init pxa9xx_init(void)
{
	...
	if (cpu_is_pxa935() || cpu_is_pxa950()) {
		...
		clks_register(pxa9xx_clkregs, ARRAY_SIZE(pxa9xx_clkregs));
	}

	return 0;
}

core_initcall(pxa9xx_init);

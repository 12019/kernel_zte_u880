...
*/
static int (*spi_send)(struct pxa910fb_info *, void *, int , unsigned int);
...
*/
static int tpo_lcd_power(struct pxa910fb_info *fbi, unsigned int spi_gpio_cs, unsigned int spi_gpio_reset, int on)
{
	int err = 0;
	...
	if (on) {
		...
		/*{
			struct pxa910fb_spi spi = {
				.tx_d = 0x87,
				.tx_len = 8,
				.rx_len = 8,
				.spi_in_data_pin = 0,
			};
			err = pxa910fb_spi_write_read(fbi, &spi, spi_gpio_cs);
			printk("err %d, reg 0x87 is %x\n", err, spi.rx_d);
		}*/
	} else  {
...
static struct pxa910fb_mach_info dkb_tpo_lcd_info __initdata = {
	.id                     = "Base",
	.modes                  = video_modes,
...
static struct pxa910fb_mach_info dkb_tpo_lcd_ovly_info __initdata = {
	.id                     = "Ovly",
	.modes                  = video_modes,
	.num_modes              = ARRAY_SIZE(video_modes),
	.pix_fmt                = PIX_FMT_RGB565,
...
{
	struct pxa910fb_mach_info *fb = &dkb_tpo_lcd_info, *ovly = &dkb_tpo_lcd_ovly_info;

/*
 * linux/arch/arm/mach-mmp/include/mach/pxa910fb.h
 *
...
#ifndef __ASM_MACH_PXA910FB_H
#define __ASM_MACH_PXA910FB_H
/* ---------------------------------------------- */
/*              Header Files                      */
/* ---------------------------------------------- */
#include <linux/fb.h>
...
#define FBIO_WAIT_VSYNC_ON                   _IO(FB_IOC_MAGIC, 20)
#define FBIO_WAIT_VSYNC_OFF                  _IO(FB_IOC_MAGIC, 21)
...
/* The pxa910_fb_chroma structure is here for legacy compatibility with former 
...
#define DSI2DPI		1
#define DSI		2
#define SMART_PANEL	3

#if defined(CONFIG_PXA910_SMART_LCD)
...
struct pxa910fb_info {
	...
	struct mutex		lcd_init;/*zhengxiaohong added 20110131*/
	...
	void (*lcd_xxx_trans_dma)(struct pxa910fb_info *fbi);
	#endif
};
...
struct pxa910fb_mach_info {
	...
	*/
	 ...
	 */
	int (*pxa910fb_lcd_power)(struct pxa910fb_info *, unsigned int, unsigned int, int);
	/*
	 * dsi to dpi setting function
	 */
	void (*dsi2dpi_set)(struct pxa910fb_info *);
	#if defined CONFIG_PXA_U802
	void (*lcd_xxx_trans_dma)(struct pxa910fb_info *fbi);
       #endif
};

/* If all LCD related MFP pins are all configured with function(1), spi_in_data_pin should be set.
 * If SPI is from other MFP configuration, such as function(2)/(3), spi_in_data_pin should be clear. */
struct pxa910fb_spi {
	u32 tx_d;
	u32 rx_d;
	u8 tx_len;
...
extern struct pxa910fb_info *pxa910fbi[PXA910FB_FB_NUM];
extern int wait_for_vsync(struct pxa910fb_info *fbi);
extern int pxa910fb_spi_send(struct pxa910fb_info *fbi, void *cmd, int count, unsigned int spi_gpio_cs);
extern int pxa910fb_spi_write_read(struct pxa910fb_info *fbi, struct pxa910fb_spi *spi, unsigned int spi_gpio_cs);

/* Before this function call
 * mfp83 should be configured as LCD control signal (mfp func 1)
 * mfp82 should be configured as gpio and be requested as gpio output */
extern int pxa910fb_smart_cmd(struct pxa910fb_info *fbi, unsigned short *cmd, unsigned int num);
#endif /* __KERNEL__ */
#endif /* __ASM_MACH_PXA910FB_H */

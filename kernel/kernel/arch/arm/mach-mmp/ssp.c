...
int ssp_init(struct ssp_dev *dev, u32 port, u32 init_flags)
{
	...
	if (ssp == NULL)
		{printk(KERN_ERR "ssp_request failed\n");
		return -ENODEV;}
...
static struct platform_device_id ssp_id_table[] = {
...
static struct platform_driver pxa910_ssp_driver = {
	.driver		= {
		.name	= "pxa910-ssp",
	},
	.probe		= ssp_probe,
...
static int __init pxa_ssp_init(void)
{
	...
	if (ret) {
		printk(KERN_ERR "failed to register ssp_driver");
		return ret;
	}

	ret = platform_driver_register(&pxa910_ssp_driver);
	if (ret) {
		printk(KERN_ERR "failed to register pxa910_ssp_driver");
	}
	return ret;
}

...
static void mspm_do_idle(void)
{
	...
	if (!need_resched()) {
		...
		record_idle_stats();
#endif
	/*oops: this may cause LPM bit not clear at once, just ignore msec*/
	//	msec = get_remain_slice();
		if(find_first_bit(lowpower_devices, DVFM_MAX_CLIENT)
				!= DVFM_MAX_CLIENT)
			goto extidle;
	...
	extidle:
		if (lpidle_is_valid(enable_deepidle, msec, &freqs,
			...
			printk("\n\nOops! external idle is not allowed!\n\n");
		}
out:
		mspm_cpu_idle();
		if (__raw_readl(MPMU_AWUCRS) & BIT_23)
			__raw_writel(__raw_readl(MPMU_SD_ROT_WAKE_CLR) | BIT_1, MPMU_SD_ROT_WAKE_CLR);
#ifdef DEBUG_IDLE_STATUS
		update_idle_stats();
#endif
	}

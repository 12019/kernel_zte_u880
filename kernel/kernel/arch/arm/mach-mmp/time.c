...
static inline uint32_t timer_read(void)
{
	...
	do {
		...
		val2 = __raw_readl(TIMERS_VIRT_BASE + TMR_CR(0));
	} while (val2 != val);
#else
				...
				struct clock_event_device *dev)
{
	...
	reg = __raw_readl(APBC_PXA168_TIMERS);
	__raw_writel(reg | 0x10, APBC_PXA168_TIMERS);
	reg = __raw_readl(TIMERS_VIRT_BASE + TMR_CCR);
...
static void __init timer_config(void)
{
	...
	cmr = __raw_readl(TIMERS_VIRT_BASE + TMR_CMR);
	ccr = __raw_readl(TIMERS_VIRT_BASE + TMR_CCR);
	cer = __raw_readl(TIMERS_VIRT_BASE + TMR_CER);

	__raw_writel(cer & ~0x2, TIMERS_VIRT_BASE + TMR_CER); /* disable */
	__raw_writel(0x7, TIMERS_VIRT_BASE + TMR_ICR(1));  /* clear status */
	__raw_writel(0x0, TIMERS_VIRT_BASE + TMR_IER(1));

	ccr |= TMR_CCR_CS_1(0x1);
	__raw_writel(ccr, TIMERS_VIRT_BASE + TMR_CCR);

	__raw_writel(0x0, TIMERS_VIRT_BASE + TMR_PLCR(1)); /* free-running */
	__raw_writel(0x0, TIMERS_VIRT_BASE + TMR_PLVR(1));
	__raw_writel(cmr | 0x2, TIMERS_VIRT_BASE + TMR_CMR);
}
...
static void __init pxa910_timer_init(void)
{
	...
	timer_init(IRQ_PXA910_AP1_TIMER2);
}
...
static void pxa910_tmr_save(struct tmr_regs *context, unsigned int tmr_base)
{
...
static void pxa910_tmr_restore(struct tmr_regs *context, unsigned int tmr_base)
{
	unsigned int i, j, temp;
...
{
//	pxa910_tmr_save(&tmr_saved_regs, TIMERS1_VIRT_BASE);
}

static void pxa_timer_resume(void)
{
//	pxa910_tmr_restore(&tmr_saved_regs, TIMERS1_VIRT_BASE);
}
#else

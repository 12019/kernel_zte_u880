...
static u16 prox_threshold_param = 0x1840;//2F40
...
#if 0
			reg_val = taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x01);
			if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x01), reg_val|0xF6))) < 0) 
			{
                        TSL2771_DBG( "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_on\n");
                        return (ret);
			}
	
			reg_val = taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x13);
			TSL2771_DBG("!!!!!!!!!!!status is 0x%x!!!!!!!!!!", reg_val);
#endif
//                       if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x02), 0xEE))) < 0)
			if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x02), 0xFF))) < 0)
			{
                        TSL2771_DBG( "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_on\n");
                        return (ret);
			}
            TSL2771_DBG("========> 1 is 0x%x!!!!!!!!!!", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x02));
            ...
            TSL2771_DBG("========> 2 is 0x%x!!!!!!!!!!", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x03));
//qiu-threshold
#if 0
                        ...
                        if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x0D), 0x00))) < 0)
			{
                        ...
                        if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x0E), 0x01))) < 0)
#else
                        if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x0E), 0x08))) < 0)
#endif
			{
                        TSL2771_DBG( "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_on\n");
                        return (ret);
...
//			if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x00), (0x2D))) < 0))
			if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x00), (0x0D | reg_val))) < 0))
			{
                        ...
                        TSL2771_DBG("========> 11 is 0x%x!!!!!!!!!!", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x00));

//			reg_val = taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | TAOS_TRITON_CNTRL);
//			TSL2771_DBG("open PROX reg_val = 0x%x\n", reg_val);
			// TBD - start polling kernel timer
			break;
...
//qiu
			...
			reg_val = taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | TAOS_TRITON_CNTRL);
//            if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x00), reg_val&(0xfc)))) < 0)
//			if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x00), (reg_val&0xDB)|0x10))) < 0)
			if ((ret = (taos_i2c_write_byte_data((TAOS_TRITON_CMD_REG|0x00), (reg_val & 0xFB)))) < 0)
			{
                 //   TSL2771_DBG( "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_off\n");
                    return (ret);
            }
	...
	int ret;
        ...
        if (alsps->alsps_switch & PROXIMITY_ENABLE)
        {
               ...
               //clear_bit(ALSPS_PROXIMITY_DATA_READY, &ptr->status);
				TSL2771_DBG("******** Now report pro data *******, data = %d", data.value);
                input_report_abs(in_pro->idev, ABS_DISTANCE, data.value);
                input_sync(in_pro->idev);
/*
...
*/
	...
	TSL2771_DBG("\n\n0x00 reg value = 0x%x\n", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x00));
	TSL2771_DBG("0x01 reg value = 0x%x\n", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x01));
	TSL2771_DBG("0x02 reg value = 0x%x\n", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x02));
	TSL2771_DBG("0x0D reg value = 0x%x\n", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x0D));
	TSL2771_DBG("0x0E reg value = 0x%x\n", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x0E));
	TSL2771_DBG("0x12 reg value = 0x%x\n\n", taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x12));
#endif
	struct input_event data;
...
//     	data.value = (((reg19 << 8) | reg18) >> 2);
		data.value = ((reg19 << 8) | reg18);

		TSL2771_DBG("******** Now report pro data *******, data = %d\n", data.value);
        input_report_abs(idev, ABS_DISTANCE, data.value);
        input_sync(idev);
	} 
}
...
//        input_event(idev, EV_LED, LED_MISC, data.value);
...
static void proximity_power_on(void)
{
	...
	if (ptr == NULL) {
		TSL2771_DBG("alsps: proximity open kzalloc failed!\n");
		return -ENOMEM;
	}

	if (!alsps->proximity_count) 
	{
		taos_ioctl(TAOS_IOCTL_PROX_ON, NULL); 
		alsps->alsps_switch |= PROXIMITY_ENABLE;//opened by Huangxin
	}
    
	clear_bit(ALSPS_PROXIMITY_DATA_READY, &ptr->status);
	set_bit(ALSPS_PROXIMITY_FIRST_POLL, &ptr->status);
	filep->private_data = ptr;
		
	disable_irq(alsps->irq);
	list_add(&ptr->list, &proximity_process_list);
	++alsps->proximity_count;
	enable_irq(alsps->irq);
...
static void proximity_power_off(void)
{
	...
	if (--alsps->proximity_count <= 0) 
	{
		...
		TSL2771_DBG("Proximity close successfully\n");
	} 
	
//6.10
//	mod_timer(&alsps_timer_after_proximity_close, jiffies + msecs_to_jiffies(5000));
	...
	if (alsps->ambient_count == 0) 
	{
		if (flag == 0)
		{
		    if (timer_status == TIMER_OFF)
		    {
		       ...
		       TSL2771_DBG("Ambient close successfully.\n");
		    } 
		   else
	               TSL2771_DBG("Timer is on, do not open ambient\n");
	        }
...
{
	unsigned char chip_id = taos_i2c_read_byte_data(TAOS_TRITON_CMD_REG | 0x12);
...
{
	struct proc_dir_entry *alsps_proc_file = create_proc_entry("driver/alsps_chip_id", 0644, NULL);
	...
	if (alsps_proc_file) 
	{
		...
		TSL2771_DBG("alsps proc file create failed!\n");
}

static int __devinit alsps_probe(struct platform_device *pdev)
{
...
*/
	TSL2771_DBG("Start %s",__FUNCTION__);
        ...
        if (!pdev)
        {
                TSL2771_DBG("alsps: platform device - null pointer!\n");
                goto failed;
        }
	...
	if (!alsps) 
	{
...
#if 0
        /* added by Huangxin2011_02_15 for input event*/
        in_am = (struct input_function *)kzalloc(sizeof(struct input_function), GFP_KERNEL);
        in_pro = (struct input_function *)kzalloc(sizeof(struct input_function), GFP_KERNEL);

        in_am->idev = input_allocate_device();
        in_pro->idev = input_allocate_device();
        ...
        if (in_pro->idev == NULL) 
		{
            dev_err(&pdev->dev, "Failed to allocate inpuit device!\n");
	       	ret = -ENOMEM;
            goto out_pro;
        }

        in_am->idev->name = "ambient";
        in_pro->idev->name = "proximity";
         
        set_bit(EV_ABS, in_pro->idev->evbit);
        in_pro->idev->absbit[0] = BIT(ABS_DISTANCE);     
       //input_set_abs_params(alsps->idev, ABS_DISTANCE, 0, , 0, 0);        
        set_bit(EV_LED, in_am->idev->evbit);
        in_am->idev->ledbit[0] = BIT(LED_MISC);
       // input_set_abs_params(in_am->idev, ABS_RX, 0, 23040, 0, 0);

        ret = input_register_device(in_am->idev);
        if (ret) 
		{
        ...
        ret = input_register_device(in_pro->idev);
        if (ret) 
		{
            dev_err(&pdev->dev, "Can't register input device: %d\n", ret);
            goto out_reg_pro;
		...
		sensor_create_sysfs(in_pro->idev, INPUT_PROXIMITY_SENSOR);
		sensor_create_sysfs(in_am->idev, INPUT_AMBIENT_SENSOR);
...
//			  IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING, "alsps", NULL);
	...
	dev_set_drvdata(&pdev->dev, alsps);
...
//	register_early_suspend(&alsps_early_suspend_desc);
	...
	taos_cfgp->prox_threshold = prox_threshold_param;
...
static int __devexit alsps_remove(struct platform_device *pdev)
{
	if (alsps) {
		free_irq(alsps->irq, NULL);
	}

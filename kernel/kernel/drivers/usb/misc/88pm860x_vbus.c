...
#else
extern void cdrom_delayed_work(void);
#endif
...
static irqreturn_t pm860x_vbus_handler(int irq, void *data)
{
	...
	if (ret < 0)
		goto out;
	if ((ret & STATUS2_VBUS)&&(ret & STATUS2_CHG))
		{
		printk("pm860x_vbus_handler:VBUS_HIGH\n");
		ret = VBUS_HIGH;
		}
	else
		{
		ret = VBUS_LOW;
		printk("pm860x_vbus_handler:VBUS_LOW\n");
			#ifdef CONFIG_USB_CDROM	
...
static int pm860x_query_usbid(void)
{
 ...
 struct delayed_work	charger_init_detect_work;
extern void handle_charger_init_detect_work(void);
...
static int __devinit pm860x_vbus_probe(struct platform_device *pdev)
{
	...
	vbus->irq_status += chip->irq_base;

	vbus->idpin = pdata->idpin;

	vbus->i2c = (chip->id == CHIP_PM8607) ? chip->client : chip->companion;
	INIT_DELAYED_WORK(&charger_init_detect_work, handle_charger_init_detect_work);

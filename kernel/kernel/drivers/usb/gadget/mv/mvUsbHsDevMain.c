...
extern uint_8 acusb_judge_flag;
#ifdef CONFIG_CHARGER_88PM860X
extern void usb_ac_state_set(uint_32 state);
#endif
/*FUNCTION*-------------------------------------------------------------
*
...
{ /* Body */
   ...
   if (status & EHCI_STS_RESET) 
   {
	...
	#ifdef CONFIG_CHARGER_88PM860X
	usb_ac_state_set(EHCI_STS_RESET);
	//printk(KERN_INFO"_usb_isr RESET enumeration_result:500MA--- \n");
	#endif	   
   } /* Endif */
   ...
   if (status & EHCI_STS_INT) 
   {
	...
	if(acusb_judge_flag)
	{
		usb_ac_state_set(EHCI_STS_SUSPEND);
		//printk(KERN_INFO"_usb_isr SUSPEND enumeration_result:500MA--- \n");
	}
	#endif
...
void _usb_dci_vusb20_start(_usb_device_handle handle)
{
   ...
   {
   	//usb1.1
   	 _usb_dci_vusb20_set_speed_full(handle, 0);
   }   
...
uint_32 _usb_dci_irq_reg (_usb_device_handle handle)
{	
	USB_DEV_STATE_STRUCT_PTR     usb_dev_ptr;	
	VUSB20_REG_STRUCT_PTR        dev_ptr;	
	uint_32                      status;	

...
#define PRODUCT_ID		0x1382
extern unsigned char launch_start;
extern unsigned char zte_prodtestmod;
#ifdef CONFIG_USB_CDROM	
unsigned char cdrom_start_stop_flag=0;;
char cur_usbcfg[100];
...
static struct usb_device_descriptor device_desc = {
	...
	defined(CONFIG_USB_ANDROID_MARVELL_MODEM) || \
...
int android_switch_function(char *func)
{
	...
	int product_id;
	#ifdef CONFIG_USB_CDROM	
	char *cdrom_usbcfg="usb_mass_storage";
	static unsigned char cfginit=0;
	#endif
	if (!func) {
	...
	#ifdef CONFIG_USB_CDROM	
	if( zte_prodtestmod==0 )
	{
		if(cfginit==0)
		{
			memset(cur_usbcfg, 0x00, 100);
			memcpy(cur_usbcfg, func, strlen(func));
			cfginit=1;
		}

		if(get_cdrom_function())
		{
			memset(cur_usbcfg, 0x00, 100);
			memcpy(cur_usbcfg, func, strlen(func));
			//printk( "cur_usbcfg(%s)\n",cur_usbcfg);
			
			if( strstr(func, "rndis") )
			{
			}
			else
			{
				if(cdrom_start_stop_flag==0)
				{
					memset(func, 0x00, strlen(func));
					memcpy(func, cdrom_usbcfg, strlen(cdrom_usbcfg));
					//printk( "cdrom_usbcfg(%x) func(%s)\n",strlen(cdrom_usbcfg),func);
				}
			}
		}
	}
	...
	usb_gadget_re_enum(dev->cdev->gadget);
#else
	/* force reenumeration */
...
#ifdef CONFIG_USB_CDROM	
void android_switch_cur_function(void)
{
	char usbswitchcfg[100];
	printk( "android_switch_cur_function\n");
	memset(usbswitchcfg, 0x00, 100);
	memcpy(usbswitchcfg, cur_usbcfg, strlen(cur_usbcfg));
	android_switch_function(usbswitchcfg);
}
EXPORT_SYMBOL(android_switch_cur_function);
#endif
	...
	if( zte_prodtestmod==0 )
	{
		...
		{		
			if(cdrom_start_stop_flag==0)
			{
				if(enable)
				{
					//printk(KERN_ERR"android_enable_function return\n");
					return;
				}
			}
		}

...
static void set_dvfm_constraint(struct sdhci_host *host)
{
	unsigned long flags;
...
static void unset_dvfm_constraint(struct sdhci_host *host)
{
	unsigned long flags;
...
static void sdhci_dumpregs(struct sdhci_host *host)
{
	...
	printk(KERN_DEBUG DRIVER_NAME ": %d, %s\n", host->mmc->index, mmc_hostname(host->mmc));
...
#ifdef CONFIG_MMC_PXA_SDH
	printk(KERN_DEBUG DRIVER_NAME ": Command: 0x%08x\n",
		sdhci_readw(host, SDHCI_COMMAND));
	printk(KERN_DEBUG DRIVER_NAME ": Fifo_param:0x%08x | Burst set: 0x%08x\n",
		sdhci_readw(host, SD_FIFO_PARAM),
		sdhci_readw(host, SD_CLOCK_AND_BURST_SIZE_SETUP));
#endif
	printk(KERN_DEBUG DRIVER_NAME ": ===========================================\n");
}

#ifdef CONFIG_DEBUG_FS
void mmc_dump_regs(struct mmc_host *mmc)
{
...
void mmc_trigger_sdio_irq(struct mmc_host *mmc)
{
	struct sdhci_host *host;
	unsigned long flags;
...
void mmc_rescan_bus(struct mmc_host *mmc)
{
	struct sdhci_host *host;
...
static void sdhci_specific_sdio(struct mmc_host *mmc, int enable)
{
	struct sdhci_host *host;
	unsigned long flags;
...
static const struct mmc_host_ops sdhci_ops = {
	...
	.specific_sdio		= sdhci_specific_sdio,
#ifdef CONFIG_DEBUG_FS
	.dump_regs		= mmc_dump_regs,
	.trigger_sdio_irq	= mmc_trigger_sdio_irq,
	.rescan_bus		= mmc_rescan_bus,
#endif
};
...
int sdhci_suspend_host(struct sdhci_host *host, pm_message_t state)
{
	...
	if (host->mmc->card && host->mmc->card->pending_interrupt) {
		DBG("%s wake up event before free ireq, resume\n",mmc_hostname(host->mmc));
	...
	host->int_mask_bk = sdhci_readl(host, SDHCI_INT_ENABLE);
...
int sdhci_resume_host(struct sdhci_host *host)
{
	...
	sdhci_clear_set_irqs(host, 0, host->int_mask_bk);
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if (ret) {
		printk(KERN_ERR "Error %d: Fails to register %s into dvfm.\n", ret, mmc_hostname(host->mmc));
		goto untasklet;
	}
#endif

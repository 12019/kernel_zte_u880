			    ...
			    struct mtd_oob_ops *ops)
{
	...
	if (ret)
	{
		printk(KERN_ERR"read fail return(%x) (%x)\n",ret, page);
		return ret;
	}

	if (mtd->ecc_stats.failed - stats.failed)
	{
		printk(KERN_ERR"read return EBADMSG(%x) (%x) (%x)\n",page, mtd->ecc_stats.failed, stats.failed);
		return -EBADMSG;
	}
	
	if (mtd->ecc_stats.corrected - stats.corrected)
		printk(KERN_ERR"mtd->ecc_stats.corrected is(%x) (%x) (%x)\n",page, mtd->ecc_stats.corrected, stats.corrected);

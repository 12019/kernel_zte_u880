...
static void pxa27x_keypad_build_keycode(struct pxa27x_keypad *keypad)
{
	...
	__set_bit(352, input_dev->keybit);
#endif
		...
		const char __user *buffer,  unsigned long count, void *data)
{
	...
	static char input[PAGE_SIZE]; /*for stack limitation */
	int key;
	int i;
...
static int __devinit pxa27x_keypad_probe(struct platform_device *pdev)
{
	...
	input_dev->evbit[0] = BIT_MASK(EV_KEY);// | BIT_MASK(EV_REP);//added by zhanglixiao,zlx_kbd_1210
	...
	//
#ifdef TS_KEY_REPORT
	input_keypad_dev = keypad->input_dev ;
#endif

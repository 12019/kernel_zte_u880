...
//	if (type != INPUT_G_SENSOR) 
	if ((type != INPUT_G_SENSOR) && (type != INPUT_PROXIMITY_SENSOR) && (type != INPUT_AMBIENT_SENSOR)) 
	{
...
//	printk("****** sensor: add device of %s ******\n", name);
	...
	if (!sensor_input_idev_ext)
		return -ENOMEM;
/*
	if(type == INPUT_ORIENTATION_SENSOR)
	{
		if(NULL == ori_input_dev)
		{
			printk("null ori_input_dev\n");
			return err;
		}

		sensor_input_idev_ext = ori_input_dev;
	}
*/
//huangxin added 
	...
	if (type == INPUT_G_SENSOR) {
		...
		__set_bit(EV_ABS, sensor_input_idev_ext->evbit);
//		sensor_input_idev_ext->ledbit[0] = BIT(LED_MISC);
...
//		sensor_input_idev_ext->absbit[0] = BIT(ABS_DISTANCE);
...
/******* start-20110221 added by qmm *******
	else if (type == INPUT_ORIENTATION_SENSOR) 
	{
		printk("****** Now come into ori sensor_input_add ******\n");

...
static void unset_dvfm_constraint(struct cyttsp *p)
{
	//spin_lock_irqsave(&p->dvfm_lock.lock, p->dvfm_lock.flags);
...
{
	struct cyttsp *p_work =  
		container_of(work, struct cyttsp, unset_dvfm_work);
...
{
	struct cyttsp *p_work =
		container_of(work, struct cyttsp, set_dvfm_work);
	set_dvfm_constraint(p_work);
}

static void dvfm_timer_handler(unsigned long data)
{
	struct cyttsp *p = (struct cyttsp *)data;
...
int ttsp_write_block_data(struct cyttsp *ts, u8 command,
	u8 length, void *buf)
{
	int rc;
	int tries;
...
no_track_id:
	...
	for (id = 0; id < CY_NUM_MT_TCH_ID; id++) {
		...
		if (t->snd_trk[id] < CY_NUM_TRK_ID) {
			if(ts_debug)
			{
				printk("CY: x 0x%x, y 0x%x, id 0x%x, w 0x%x\n", 
					t->cur_mt_pos[t->snd_trk[id]][CY_XPOS], t->cur_mt_pos[t->snd_trk[id]][CY_YPOS], 
					id, t->tool_width);
			}
			input_report_abs(ts->input, ABS_MT_TOUCH_MAJOR,
...
exit_xy_worker:
	/* reset the watchdog */
	reset_watchdog();//mod_timer(&ts->timer, jiffies + CY_WDG_TIMEOUT);//
	DBG(printk(KERN_INFO"%s: finished.\n", __func__);)
	return;
}
static void cyttsp_worker(struct work_struct *work)
{
	struct cyttsp *ts = container_of(work, struct cyttsp, wq_work);
...
static irqreturn_t cyttsp_irq(int irq, void *handle)
{
	...
	if (!work_pending(&ts->wq_work))
	{
		queue_work(ts->wq, &ts->wq_work);
	}
	else
	{
		if(point_num % 60 == 0)
...
static struct proc_dir_entry *cyttsp_touch_proc_file;

//static ssize_t proc_read_val(char *page, char **start, off_t off, int count, int *eof,
//	  void *data)
    ...
    char __user *buffer, size_t count, loff_t *offset)
{
	...
	len += sprintf(buffer_synap + len, "This is Cypress Touchscreen\n");
...
static struct file_operations cyttsp_touch_proc_ops = {
	.read = proc_read_val,
	.write = proc_write_val,
};

static void create_cyttsp_touch_proc_file(void)
{
	cyttsp_touch_proc_file =
	    create_proc_entry(TOUCH_PROC_FILE, 0777, NULL);
	if (cyttsp_touch_proc_file) {
		cyttsp_touch_proc_file->proc_fops = &cyttsp_touch_proc_ops;
	} else
		printk(KERN_INFO "proc file create failed!\n");
}

static void remove_cyttsp_touch_proc_file(void)
{
...
{
	//schedule_work(&mod_timer_work);
        //
	mod_timer(&g_ts->timer, jiffies + CY_WDG_TIMEOUT);
}

static void cyttsp_mod_timer(struct work_struct *work)
{
	mod_timer(&g_ts->timer, jiffies + CY_WDG_TIMEOUT);
}
...
static void cyttsp_check_bl(struct work_struct *work)
{
	...
	if (retval < 0) {
		...
		if (ts->platform_data->wakeup && resumed) {
...
static int cyttsp_resume(struct cyttsp *ts)
{
	...
	//printk("%s\n", __FUNCTION__);
        //
	printk(KERN_INFO"%s: Power state is %d, %s\n",
		__func__, ts->platform_data->power_state, (ts->platform_data->power_state == CY_ACTIVE_STATE) ?
		"ACTIVE" : "IDLE");
	...
	if(1){
		...
		//for touchscreen invalid when wakeup cl0421
		//cmd = CY_OPERATE_MODE;
		//ttsp_write_block_data(ts, CY_REG_BASE, sizeof(cmd), &cmd);
		if (!(retval < 0)) {
			retval = ttsp_read_block_data(ts, CY_REG_BASE,
...
static int cyttsp_suspend(struct cyttsp *ts)
{
	...
	if (ts->platform_data->use_sleep &&
			(ts->platform_data->power_state == CY_ACTIVE_STATE)) {
		sleep_mode = CY_DEEP_SLEEP_MODE;//for touchscreen invalid when wakeup cl0421
		retval = ttsp_write_block_data(ts,
			...
			if (ts->reg_id == 0x00) {
				...
				case GET_HSTMODE(CY_OPERATE_MODE):
					...
					ts->platform_data->power_state =
						CY_ACTIVE_STATE;
					enable_irq(ts->irq);
					reset_watchdog();//mod_timer(&ts->timer, CY_WDG_TIMEOUT);
					printk(KERN_INFO "%s: "
...
void *cyttsp_core_init(struct cyttsp_bus_ops *bus_ops, struct device *pdev)
{
	...
	if (!dvfm_register(CY_I2C_NAME, &ts->dvfm_dev_idx)) {
	...
	INIT_WORK(&ts->wq_work, cyttsp_worker);
	ts->wq = create_singlethread_workqueue(CY_I2C_NAME);
	if (!ts->wq) {
		printk("%s:failed to create work queue.\n", __func__);
		goto error_input_allocate_device;
	}
	
	/* setup watchdog */
	INIT_WORK(&ts->work, cyttsp_check_bl);
	INIT_WORK(&mod_timer_work, cyttsp_mod_timer);//
	g_ts = ts;//
...
void cyttsp_core_release(void *handle)
{
	...
	if( ts->dvfm_dev_idx >= 0 ) {
		...
		dvfm_unregister(CY_I2C_NAME, &ts->dvfm_dev_idx);
		ts->dvfm_dev_idx = -1;
	}
#endif

#ifdef CONFIG_PROC_FS
	remove_cyttsp_touch_proc_file();
#endif
	kfree(ts);
}
MODULE_LICENSE("GPL");

...
static struct page *__ire_follow_page(struct mm_struct *mm, unsigned long address);
...
static int ire_submit(struct ire_context *ctx, struct ire_submit_req submit_req)
{
	...
	y_paddr = dma_map_page(dev->dev, page, src->y_vaddr % PAGE_SIZE, submit_req.size, DMA_TO_DEVICE);
	if(!y_paddr){
	...
	y_paddr = dma_map_page(dev->dev, page, dst->y_vaddr % PAGE_SIZE, submit_req.size, DMA_FROM_DEVICE);
	if(!y_paddr){
...
static int ire_desubmit(struct ire_context *ctx, unsigned int buf_id)
{
	...
	mutex_lock(&dev->s_mutex);
	dma_unmap_page(dev->dev, dev->buf_pair[buf_id].src.y_paddr, dev->buf_pair[buf_id].src.size, DMA_TO_DEVICE);
	dma_unmap_page(dev->dev, dev->buf_pair[buf_id].dst.y_paddr, dev->buf_pair[buf_id].src.size, DMA_FROM_DEVICE);
...
{
	struct ire_device *dev = ctx->dev;
	unsigned long y_pitch = 0, uv_pitch = 0;
	unsigned long yp0_pitch = 0, uvp0_pitch = 0;
	int ret = 0;
	...
	if ((fmt->fmt == IRE_PIXFMT_YUV422_PACKED_UYVY)
		...
		if(fmt->rot_angle == IRE_ROT_180)
			ire_write(dev, fmt->fmt | 2 << 21 | fmt->rot_angle << 11 | 0 << 30 | 1 << 29,
	       	IRE_CTRL0);
		else
			ire_write(dev, fmt->fmt | 1 << 21 | fmt->rot_angle << 11 | 0 << 30 | 1 << 29,
	       	IRE_CTRL0);
	}else{
		ire_write(dev, fmt->fmt | fmt->rot_angle << 11 | 0 << 30 | 1 << 29,
		       	IRE_CTRL0);
	}

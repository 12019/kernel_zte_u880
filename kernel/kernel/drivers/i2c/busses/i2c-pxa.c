...
#define ILCR_FLV_MASK		(0x1FF << 9)	   /* clk frequence ajust for FS mode */
...
static void i2c_pxa_scream_blue_murder(struct pxa_i2c *i2c, const char *why)
{
	...
	printk(KERN_ERR "i2c: ICR: %08x ISR: %08x IBMR: %08x\n",
...
static int i2c_pxa_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	enable_irq(i2c->irq);
	/*
	 * fix me: hard coding to enable irq
	 * for we don't use set_wake, enable, disable
	 * callbacks for icu_irq_chip, the new
	 * arch has been implemented on .35 kernel
	 */
	__raw_writel(IRQ_ROUTE_TO_AP | PRIORITY_DEFAULT,\
			ICU_INT_CONF(i2c->irq));
 ...
 out:
	...
	disable_irq(i2c->irq);
	/* fix me: hard coding to disable irq */
	__raw_writel(PRIORITY_NONE, ICU_INT_CONF(i2c->irq));
...
static int i2c_pxa_probe(struct platform_device *dev)
{
	...
	/*setting i2c fast mode clk about 330k*/
	ilcr = readl(_ILCR(i2c));
	/*ilcr =(ilcr&~(ILCR_FLV_MASK))|(8<<9);*/ /*about 380k*/
	ilcr =(ilcr&~(ILCR_FLV_MASK))|(20<<9);
	writel(ilcr, _ILCR(i2c));

/*
 * linux/drivers/video/pxa910fb_ovly.c -- Marvell PXA910 LCD Controller
 *
 * Copyright (C) Marvell Semiconductor Company.  All rights reserved.
 *
 * 2009-03-19   adapted from original version for PXA910
...
static struct _sOvlySurface gOvlyInfo;
static int ovly_dma_on = 0;
 #if ( defined( CONFIG_PXA_U802))
 int pending_colorkey_wr = 0;
 int pending_colorkey_val = 0;
 int pending_vid_switch = 0;
 extern volatile int dma_tiger;
 #endif
#ifdef CONFIG_PXA_U880
extern int  lcd_type;
#endif
static int pxa910fb_set_par(struct fb_info *fi);
...
static int pxa910fb_pan_display(struct fb_var_screeninfo *var, struct fb_info *fi);
static void ovly_dma_switch(struct pxa910fb_info *fbi, int vid_on);
 ...
 *    waitlist: wait queue which indicates "using" buffer, waitlist[0] is write in DMA register
 *    current: buffer on show
 * Operation:
 ...
 *    buffers are protected irq disable/enable
 *    suspend: when lcd is suspend or overlay dma is switched off,  move all buffers as "switched"
 */
...
static unsigned int pxa910fb_rw_all_regs(struct pxa910fb_info *fbi,
...
static void pxa910_sync_colorkey_structures(struct pxa910fb_info *fbi, int direction)
{
        struct _sColorKeyNAlpha *colorkey = &fbi->ckey_alpha;
        struct pxa910_fb_chroma *chroma = &fbi->chroma;
        unsigned int temp;
...
static u32 pxa910fb_ovly_set_colorkeyalpha(struct pxa910fb_info *fbi)
{
	unsigned int rb = 0;
	unsigned int dma0 = 0;
	unsigned int temp;
	...
	if(rb){
		if (fbi->id <= 0)
			dma0 = readl(fbi->reg_base + LCD_SPU_DMA_CTRL0);
		else
			dma0 = readl(fbi->reg_base + LCD_TV_CTRL0);
	}
	...
	case FB_VID_PATH_ALPHA:
		...
		if(rb)
		rb = ((dma0 & CFG_DMA_SWAPRB_MASK)>> 4)^ (fbi->panel_rbswap);
		break;
	case FB_GRA_PATH_ALPHA:
		x |= CFG_ALPHA_MODE(0x1);
		if(rb)
		rb = ((dma0 & CFG_GRA_SWAPRB_MASK)>> 12)^ (fbi->panel_rbswap);
		break;
	...
	if (fbi->id <= 0) {
                #if ( defined( CONFIG_PXA_U802))
                pending_colorkey_wr = 1;
		pending_colorkey_val = x;
                #else
...
{
	struct pxa910fb_info *fbi = (struct pxa910fb_info *)fi->par;
	...
	if (new_info &&
		...
		fbi->surface.viewPortInfo.vPitch != new_info->vPitch)) {
			...
			if (((new_info->srcWidth * new_info->srcHeight * var->bits_per_pixel / 8) * 2) > max_fb_size) {
                                printk(KERN_ERR"%s: requested memory buffer size %d exceed the max limit %d!\n", __func__,
...
static void pxa910fb_clear_framebuffer(struct fb_info *fi)
{
        struct pxa910fb_info *fbi = fi->par;
...
static void buf_endframe(void * p)
{
	...
	if(buf){
		...
		/* Update new surface. */
		ret = check_surface(fi, pOvlySurface->videoMode,
		...
		if (1 == ret) {
			pxa910fb_set_par(fi);
                        ...
                        #else
			if (!fbi->buf_current)
				ovly_dma_switch(fbi, ovly_dma_on);
                        #endif
			...
			printk(KERN_INFO "Detect a same surface flipped in, "
				"may flicker.\n");
		}
	}
}
...
static int pxa910fb_ioctl(struct fb_info *fi, unsigned int cmd,
			unsigned long arg)
{
	...
	case FB_IOCTL_SET_VIEWPORT_INFO:
		...
		gOvlyInfo.viewPortInfo = gViewPortInfo;

		ret = check_surface(fi, -1, &gViewPortInfo, 0, 0);
		if(1 == ret){
			pxa910fb_set_par(fi);
			ret = 0;
		}
	...
	case FB_IOCTL_SET_VIDEO_MODE:
		...
		gOvlyInfo.videoMode = vmode;

		ret = check_surface(fi, vmode, 0, 0, 0);
		if(1 == ret){
			pxa910fb_set_par(fi);
			ret = 0;
		}
	...
	case FB_IOCTL_FLIP_VID_BUFFER:
	{
		...
		if (start_addr[0] && (!input_data)) {
			...
			ret = check_surface(fi, surface->videoMode,
	...
	case FB_IOCTL_GET_FREELIST:
	{
		...
		if (!(pxa910fbi[0]->active))
			buf_endframe(fi);
        
		/* Collect expired frame to list */
		collectFreeBuf(fbi->filterBufList, fbi->buf_freelist);
	...
	case FB_IOCTL_SET_VID_OFFSET:
		...
		gOvlyInfo.viewPortOffset = gViewPortOffset;

		ret = check_surface(fi, -1, 0, &gViewPortOffset, 0);
		if(1 == ret){
			pxa910fb_set_par(fi);
			ret = 0;
		}
	...
	case FB_IOCTL_SET_SURFACE:
	{
		mutex_lock(&fbi->access_ok);
		/* Get user-mode data. */
		if (copy_from_user(&gOvlyInfo, argp,
					sizeof(struct _sOvlySurface))) {
			mutex_unlock(&fbi->access_ok);
			return -EFAULT;
		}
	...
	{
		mutex_lock(&fbi->access_ok);
		ret = copy_to_user(argp, &gOvlyInfo,
			sizeof(struct _sOvlySurface));
	...
	case FB_IOCTL_SET_COLORKEYnALPHA:
		...
		pxa910fb_ovly_set_colorkeyalpha(fbi);
		break;
        case FBIOGET_CHROMAKEYS:
                pxa910_sync_colorkey_structures(fbi, FB_SYNC_COLORKEY_TO_CHROMA);
                return copy_to_user(argp, &fbi->chroma, sizeof(struct pxa910_fb_chroma)) ? -EFAULT : 0;
        case FBIOPUT_CHROMAKEYS:
                if (copy_from_user(&fbi->chroma, argp, sizeof(struct pxa910_fb_chroma)))
                        return -EFAULT;
                pxa910_sync_colorkey_structures(fbi, FB_SYNC_CHROMA_TO_COLORKEY);
                pxa910fb_ovly_set_colorkeyalpha(fbi);
                return copy_to_user(argp, &fbi->chroma, sizeof(struct pxa910_fb_chroma)) ? -EFAULT : 0;
	...
	case FB_IOCTL_SWITCH_VID_OVLY:
		...
		local_irq_save(flags);
		/*Late switch on the ovly dma when the first frame is flipped in to avoid green screen*/
                #if ( defined( CONFIG_PXA_U802))
                pending_vid_switch = 1;
	        ovly_dma_on = vid_on;
	        local_irq_restore(flags);
	        return 0;
                #else
...
static int pxa910fb_open(struct fb_info *fi, int user)
{
	u32 val;
	...
	if (fbi->id <= 0) {
		val = readl(fbi->reg_base + LCD_SPU_DMA_CTRL0);
		if((SMART_PANEL == mi->display_interface) && (val & CFG_GRA_ENA_MASK)){
			writel(0, fbi->reg_base + SPU_IRQ_ISR);
			while(!(TWC_FRAMEDONE_MASK & readl(fbi->reg_base + SPU_IRQ_ISR))){
				msleep(1);
			}
		}
	}
...
static int pxa910fb_release(struct fb_info *fi, int user)
{
        u32 val;
        struct pxa910fb_info *fbi = (struct pxa910fb_info *)fi->par;
        struct fb_var_screeninfo *var = &fi->var;
...
static int pxa910fb_check_var(struct fb_var_screeninfo *var, struct fb_info *fi)
{
	int pix_fmt;
...
static void set_graphics_start(struct fb_info *fi, int xoffset, int yoffset)
{
	...
	if (!(fbi->new_addr[0])) {
		...
		if (fbi->id <= 0) {
			...
			{
			y0 = fbi->new_addr[0] + (var->xres*(var->yres-1));
			writel(/*fbi->new_addr[0]*/y0, fbi->reg_base + LCD_SPU_DMA_START_ADDR_Y0);
			}
			else
			{
			...
			if(lcd_type==3||lcd_type==4)
			{
			v0 = fbi->new_addr[1] + (var->xres>>1)*((var->yres>>1)-1);
			u0 = fbi->new_addr[2] + (var->xres>>1)*((var->yres>>1)-1);
			writel(/*fbi->new_addr[1]*/v0, fbi->reg_base + LCD_SPU_DMA_START_ADDR_V0);
			writel(/*fbi->new_addr[2]*/u0, fbi->reg_base + LCD_SPU_DMA_START_ADDR_U0);
			}
			else
			{
                	writel(fbi->new_addr[1], fbi->reg_base + LCD_SPU_DMA_START_ADDR_V0);
			writel(fbi->new_addr[2], fbi->reg_base + LCD_SPU_DMA_START_ADDR_U0);
			}
#else
...
static int pxa910fb_set_par(struct fb_info *fi)
{
	...
	if (fbi->id <= 0) {
		...
		if(COMPAT_MODE != 0x2625) {
			...
			if(lcd_type==3||lcd_type==4)
			{
			y_mod = 800 - (fbi->surface.viewPortOffset.yOffset+yzoom);
			writel(CFG_DMA_OVSA_VLN(/*fbi->surface.viewPortOffset.yOffset*/y_mod)|
...
static int pxa910fb_fb_sync(struct fb_info *info)
{
	struct pxa910fb_info *fbi = (struct pxa910fb_info *)info->par;
...
/*
 *  pxa910fb_handle_irq(two lcd controllers)
 */
static irqreturn_t pxa910fb_handle_irq(int irq, void *dev_id)
{
	...
	isr = readl(fbi->reg_base+SPU_IRQ_ISR);
		...
		if(dma_tiger==1)
		{
			dma_tiger=0;
			if (isr & TWC_FRAMEDONE_ENA_MASK)
			writel(isr & (~TWC_FRAMEDONE_ENA_MASK),fbi->reg_base + SPU_IRQ_ISR);
			return IRQ_NONE;
		...
		wake_up(&fbi->w_intr_wq);
                #if   0
        	if (pending_colorkey_wr == 1)
		{
			//printk("zhengxiaohong_ovly:ovly IRQ: wr colorkey %x\n", pending_colorkey_val);
			tmp = readl(fbi->reg_base + LCD_SPU_DMA_CTRL1);
			tmp = tmp & (~(CFG_COLOR_KEY_MASK | CFG_ALPHA_MODE_MASK | CFG_ALPHA_MASK));
			tmp = (tmp | pending_colorkey_val);
			writel(tmp, fbi->reg_base + LCD_SPU_DMA_CTRL1);
			pending_colorkey_wr = 0;
			pending_colorkey_val = 0;
		}
	       #endif
		buf_endframe(fi);
		if (pending_vid_switch == 1)
		{
			
			ovly_dma_switch(fbi, ovly_dma_on);
			pending_vid_switch = 0;
		}
		if (isr & TWC_FRAMEDONE_ENA_MASK)
			writel(isr & (~TWC_FRAMEDONE_ENA_MASK),
				fbi->reg_base + SPU_IRQ_ISR);
		return IRQ_HANDLED;
	}
        #else
	isr = readl(fbi->reg_base+SPU_IRQ_ISR);
	if(isr & (DUMB_FRAMEDONE_ENA_MASK | TWC_FRAMEDONE_ENA_MASK)){
		/* wake up queue. */
		atomic_set(&fbi->w_intr, 1);
		wake_up(&fbi->w_intr_wq);

		/*do buffer switch for flip*/
		buf_endframe(fi);

		if (isr & DUMB_FRAMEDONE_ENA_MASK)
			writel(isr & (~DUMB_FRAMEDONE_ENA_MASK),
				fbi->reg_base + SPU_IRQ_ISR);
...
static struct fb_ops pxa910fb_ops = {
	...
	.fb_sync	= pxa910fb_fb_sync,
	.fb_ioctl	= pxa910fb_ioctl,
};
...
static int __init pxa910fb_probe(struct platform_device *pdev)
{
	...
	fi = framebuffer_alloc(sizeof(struct pxa910fb_info), &pdev->dev);
	if (fi == NULL){
...
#ifdef FB_PM_DEBUG
	pxa910fb_rw_all_regs(fbi, g_regs, 1);
#endif
	 ...
	 */
	 if(pdev->id<=0)
		ret = request_irq(res->start, pxa910fb_handle_irq, IRQF_SHARED,
					mi->id, fi);
	 else
	 	ret = request_irq(res->start, pxa910fb_tv_handle_irq, IRQF_SHARED,
					mi->id, fi);
	if (ret < 0)
	{
		/*
		 *
		 */
		dev_err(&pdev->dev, "Failed to register pxa910fb: %d\n", ret);
		ret = -ENXIO;
		goto failed;
	}

        #if ( defined( CONFIG_PXA_U802))
	tmpDMA1 = readl(fbi->reg_base + LCD_SPU_DMA_CTRL1);
	tmpDMA1 = tmpDMA1 & (~(CFG_COLOR_KEY_MASK | CFG_ALPHA_MODE_MASK | CFG_ALPHA_MASK));
	tmpDMA1 = (tmpDMA1 | 0x03012000);
	writel(tmpDMA1, fbi->reg_base + LCD_SPU_DMA_CTRL1);
	#endif
	...
	if (fbi->id <= 0){
		if(SMART_PANEL != mi->display_interface){
			writel(readl(fbi->reg_base + SPU_IRQ_ENA) | DUMB_FRAMEDONE_ENA(1),
							fbi->reg_base + SPU_IRQ_ENA);
		}else{
			writel(readl(fbi->reg_base + SPU_IRQ_ENA) | TWC_FRAMEDONE_ENA(1),
				fbi->reg_base + SPU_IRQ_ENA);
		}
...
pxa910fb_vid_suspend(struct platform_device *pdev, pm_message_t mesg)
{
	struct pxa910fb_info *fbi = platform_get_drvdata(pdev);
	struct fb_info *fi = fbi->fb_info;

#ifdef FB_PM_DEBUG
	pxa910fb_rw_all_regs(fbi, g_regs1, 1);
#endif
...
#ifdef FB_PM_DEBUG
	pxa910fb_rw_all_regs(fbi, g_regs, 0);
#endif
...
static int __attribute__((unused))
pxa910fb_vid_resume(struct platform_device *pdev)
{
	struct pxa910fb_info *fbi = platform_get_drvdata(pdev);
	struct fb_info *fi = fbi->fb_info;
	unsigned int temp;
	...
	clk_enable(fbi->clk);
	if (pxa910fb_set_par(fi) != 0) {
		printk(KERN_INFO "pxa910fb_vid_resume(): Failed in "
				"pxa910fb_set_par().\n");
		return -1;
	}
	...
	clk_disable(fbi->clk);
	printk(KERN_INFO "pxa910fb_vid_resumed\n");
...
/*	.remove		= pxa910fb_remove,		*/
#ifdef CONFIG_PM
//	.suspend	= pxa910fb_vid_suspend,
//	.resume		= pxa910fb_vid_resume,
#endif
	.driver		= {
		.name	= "pxa910fb_ovly",
		.owner	= THIS_MODULE,

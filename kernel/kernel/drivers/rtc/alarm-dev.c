...
static DEFINE_SPINLOCK(alarm_slock);
 ...
 DECLARE_WAIT_QUEUE_HEAD(alarm_wait_queue);
 uint32_t alarm_pending;
 uint32_t alarm_enabled;
static uint32_t wait_pending;

static struct alarm alarms[ANDROID_ALARM_TYPE_COUNT];
static int pwr_off_charge_set = 0;
...
static long alarm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ANDROID_ALARM_PWR_OFF_CHARGE_SET:
		spin_lock_irqsave(&alarm_slock, flags);
		printk("ANDROID_ALARM_PWR_OFF_CHARGE_SET\n");
		pwr_off_charge_set = 1;
		spin_unlock_irqrestore(&alarm_slock, flags);
		break;
...
unsigned long flags;
	...
	if (alarm_enabled & alarm_type_mask) {
		...
		alarm_enabled &= ~alarm_type_mask;
		alarm_pending |= alarm_type_mask;
		pr_alarm(INT, "wake alarm_wait_queue %s\n", __FUNCTION__);
		wake_up(&alarm_wait_queue);
	}
	else if(1 == pwr_off_charge_set) {
		pr_alarm(INT, "pwr off charge alarm_triggered_func: type = poweroff wakeup\n");
		alarm_pending |= alarm_type_mask;
		pwr_off_charge_set = 0;
		pr_alarm(INT, "pwr off charge  wake alarm_wait_queue %s\n", __FUNCTION__);
		wake_up(&alarm_wait_queue);
	}

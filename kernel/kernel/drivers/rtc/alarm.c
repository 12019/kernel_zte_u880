...
static int alarm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	spin_unlock_irqrestore(&alarm_slock, flags);
	if (alarm_pending) {
		pr_alarm(SUSPEND,"alarm pending\n");
		err = -EBUSY;
          	return err;
	}
     if (alarm_enabled &
	...
	if (wakeup_queue) {
		...
		rtc_alarm.enabled = RTC_WAKEUP_ENABLE;
		...
		if (rtc_current_time + 1 >= rtc_alarm_time) {
			...
			rtc_alarm.enabled = RTC_WAKEUP_DISABLE;
			rtc_set_alarm(alarm_rtc_dev, &rtc_alarm);
...
static int alarm_resume(struct platform_device *pdev)
{
   ...
   if (alarm_enabled &
	...
	alarm.enabled = RTC_WAKEUP_DISABLE;
				...
				struct class_interface *class_intf)
{
	...
	printk("rtc_alarm_add_device enter\n");
	...
	printk("rtc_alarm_add_device 0000\n");

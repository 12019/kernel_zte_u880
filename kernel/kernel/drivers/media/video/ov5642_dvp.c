		...
		case V4L2_PIX_FMT_YUV420:
			...
			switch (pix->width )
			{
				case CIF_WIDTH:
					...
					break;
				case 144:
					pregs = ov5642_dvp_fmt_yuv422_qcif_rotate;
			...
			case V4L2_PIX_FMT_JPEG:
				switch (pix->width )
				{
					...
					case 1600:
						pregs = ov5642_dvp_fmt_jpeg_uxga;
						ret = ov5642_dvp_set_jpegdata_size(c, pix->bytesperline, pix->sizeimage / pix->bytesperline );
						if(ret < 0 )
							goto out;
					...
					case 1024:
						pregs = ov5642_dvp_fmt_jpeg_sxga;
						ret = ov5642_dvp_set_jpegdata_size(c, pix->bytesperline, pix->sizeimage / pix->bytesperline );
						if(ret < 0 )
							goto out;
...
static int ov5642_dvp_firmware_download(struct i2c_client *client,  __s32  value)
{
	...
	return (ret > 0) ? 0 : ret;
	#endif
return ov5642_dvp_write_array(client, ov5642_AF_firmware);
}

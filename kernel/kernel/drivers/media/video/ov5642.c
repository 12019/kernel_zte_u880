...
static struct regval_list ov5642_fmt_rgb565[] = {
	/* RGB OUTPUT */
	{0x460c, 0x20},
	{0x460b, 0x37},
	{0x471c, 0xd0},
...
static struct regval_list ov5642_yuv_720p[] = {
	...
	/* 0x3030, enable DVP */
	{0x3030, 0x0b},
	/* change fps from 15fps to 30fps */
	{0x3011, 0x08},
//	{0x3011, 0x10},
	{0x3010, 0x10},
	{0x3604, 0x60},
...
static struct regval_list ov5642_jpg[] = {
	/* 2lane 15fps */
	{0x3103, 0x93},
	{0x3008, 0x82},
	{0x3017, 0x7f},
...
static struct regval_list ov5642_res_720P[] = {
	/* 1280x720 */
	{0x5001, 0xff},
	{0x3808, 0x05},
	{0x3809, 0x00},
...
static struct regval_list ov5642_res_vga_v[] = {
	/* 480 x 640 */
	{0x3808, 0x01},
	{0x3809, 0xe0},
	{0x380a, 0x02},
...
static struct regval_list ov5642_res_d1[] = {
	/* 720 x 480 */
	{0x5001, 0xff},
	{0x3808, 0x02},
	{0x3809, 0xd0},
...
static struct regval_list ov5642_res_half_vga_v[] = {
	/* 320 x 480 */
	{0x3808, 0x01},
	{0x3809, 0x40},
	{0x380a, 0x01},
...
static struct regval_list ov5642_res_half_vga[] = {
	/* 480 x 320 */
	{0x5001, 0xff},
	{0x3808, 0x01},
	{0x3809, 0xe0},
...
static struct regval_list ov5642_res_qvga_v[] = {
	/* 240 x 320 */
	{0x3808, 0x00},
	{0x3809, 0xf0},
	{0x380a, 0x01},
...
static int ov5642_read(struct i2c_client *c, u16 reg, unsigned char *value)
{
	int ret = 0;
	u8 data;
	u8 address[2];
...
static int ov5642_write(struct i2c_client *c, u16 reg, unsigned char value)
{
	int ret = 0;
	u8 data[3];
	data[0] = reg>>8;
...
static struct ov5642_win_size {
	...
	/* half VGA v */
	{
		.width		= 320,
		.height		= 480,
		.regs           = ov5642_res_half_vga_v,
	},
	/* half VGA */
	{
		.width		= 480,
		.height		= 320,
		.regs           = ov5642_res_half_vga,
	},
	/* VGA v */
	{
		.width		= 480,
...
static int ov5642_t_flash_time(struct i2c_client *client, int time)
{
	...
	struct ov5642_info *info = i2c_get_clientdata(client);
	if (time < 0) {
		printk(KERN_NOTICE "camera: Invalid flash duration: %d\n", time);
		return -EINVAL;
	}
		...
		struct ov5642_win_size **ret_wsize)
{
	...
	if (index >= N_OV5642_FMTS){
		printk(KERN_ERR "camera: ov5642 unsupported format!\n");
		return -EINVAL;
	}
	 ...
	 */
	 for (index = 0; index < N_WIN_SIZES; index++)
		 if (pix->width == ov5642_win_sizes[index].width
			&& pix->height == ov5642_win_sizes[index].height)
			break;
	if (index >= N_WIN_SIZES){
		printk(KERN_ERR "camera: format size exceeded!\n");
		return -EINVAL;
	}

	if (ret_wsize != NULL)
		*ret_wsize = ov5642_win_sizes + index;
...
//	printk(KERN_NOTICE "camera: ov5642_g_ctrl %d\n",ctrl->value);
	if (ret >= 0)
		return 0;
	return ret;
}
...
static int ov5642_s_fmt(struct i2c_client *c, struct v4l2_format *fmt)
{
	...
	if (ret)
		return ret;
	if(V4L2_PIX_FMT_JPEG == pix->pixelformat) {
		ret = ov5642_write_array(c, ov5642_jpg);
		if(ret < 0){
			return ret;
		}
	//	ret = ov5642_write_array(c, wsize->regs);
		ret = ov5642_write_array(c, ov5642_res_5M);
		if(ret < 0){
			return ret;
		}
		...
		switch (pix->width) {
			...
			case 1280:
				ret = ov5642_write_array(c, ov5642_yuv_720p);
				if(ret < 0){
					return ret;
				}
				break;
			default:
				...
				ret = ov5642_write_array(c, ovfmt->regs);
				if(ret < 0){
					return ret;
				}
				printk(KERN_ERR" fmt = %s\n" , ovfmt->desc);
		...
		ret = ov5642_write_array(c, wsize->regs);
		printk(KERN_ERR" preview size w= %d  h=%d\n", wsize->width, wsize->height);
		if(ret < 0){
			return ret;
		}
	}
...
//	printk(KERN_NOTICE "APBC_TWSI3_CLK_RST Value=0x%x\n", *(unsigned long *)0xFE01500C);	// TWSI3_CLK value
	...
	for (i = MAX_DETECT_NUM; i > 0; --i) {
		...
		printk(KERN_ERR "camera: OmniVision 5642 detect failure, will retry\n");
	}
	/* NO OV5642 detected within limited loops */
	printk(KERN_NOTICE "camera: Abort retry, failed to detect OmniVision 5642!\n");
	ret = -ENODEV;
	goto out_free_info;
...
static int ov5642_streamoff(struct i2c_client *client)
{
	...
	/* suspend */
//	ov5642_read(client, 0x300e, &val);
//	val |= 0x02;
//	ov5642_write(client, 0x300e, val);

//	msleep(200);

	/* power off sensor, does not need this */
//	ov5642_read(client, 0x3008, &val);
//	val |= 0x40;
//	ov5642_write(client, 0x3008, val);

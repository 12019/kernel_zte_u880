...
static void set_dvfm_constraint(void)
{
	...
	dvfm_disable_op_name("624MHz", dvfm_dev_idx);
	#endif
...
static void unset_dvfm_constraint(void)
{
	...
	#ifdef  CONFIG_PXA_806M
        dvfm_enable_op_name("624MHz", dvfm_dev_idx);
	#endif
...
/*
 * reserved buffer for rubbish buf
 * for OV5642 5M jpeg sensor: 3M bytes
 * user can change this buffer size according to
 * the maxim data from sensor.
 */

static int dma_buf_size = 3 * (1 << 20);
...
struct ccic_camera
{
	...
	u8  active_flag;		/* active_flag: 0 ccic is closed, 1 ccic is working */
...
void ccic_set_clock_parallel(void)
{
	...
	clk_set_rate(rst_clk, 0x05f);  // only parallel related register will be enabled
	clk_enable(pxa168_ccic_gate_clk);
	 ...
	 * REG_CTRL1[30] should be 0
	 */
	ccic_set_clock(REG_CTRL1, 0x0800003c); 	//Kevin         //CCIC power on. bit28 (PWRDNEN) must be cleared.
	if(sensor_selected)
		ccic_set_clock(REG_CLKCTRL, 0x40000001);//52M//0x40000004	//enable MCLK
	else
	ccic_set_clock(REG_CLKCTRL, 0x40000002);	//enable MCLK
...
void ccic_set_clock_parallel_52M(void)
{
	/*
	 *	bit6 = 0 select 312MHz clock as MCLK divider
	 *	bit6 = 1 select 52MHz clock as MCLK divider
	 */
	  printk("++++++:ccic_set_clock_parallel_52M\n");
	clk_set_rate(rst_clk, 0x05f);  // only parallel related register will be enabled
	clk_enable(pxa168_ccic_gate_clk);
...
EXPORT_SYMBOL (ccic_set_clock_parallel_52M);

void ccic_set_clock_parallel_26M(void)
{
	/*
	 *	bit6 = 0 select 312MHz clock as MCLK divider
	 *	bit6 = 1 select 52MHz clock as MCLK divider
...
EXPORT_SYMBOL (ccic_set_clock_parallel_26M);
...
int ccic_sensor_attach(struct i2c_client *client)
{
	...
	if (cam->sensor_type == V4L2_IDENT_MT9V113 || cam->sensor_type == V4L2_IDENT_OV7690){
		cam->sensors[SENSOR_LOW] = client;
		cam->bus_type[SENSOR_LOW] = BUS_PARALLEL;
		detected_low = 1;
		sensor_selected = SENSOR_LOW; /* Set default sensor */
 	} else if (cam->sensor_type == V4L2_IDENT_MT9T111|| cam->sensor_type == V4L2_IDENT_OV5642
 		 || cam->sensor_type == V4L2_IDENT_OV5640 || cam->sensor_type == V4L2_IDENT_S5K5CA)
 	{ 
	...
	//		cam->bus_type[SENSOR_HIGH] = BUS_PARALLEL;
		...
		printk(KERN_ERR "camera: mt9t111 or ov5642 attached--bata10--\n");
		...
		struct v4l2_buffer *buf)
{
	...
	if ((buf->memory == V4L2_MEMORY_USERPTR)
		...
		if(buf->length < cam->pix_format.sizeimage && fmt->pixelformat != V4L2_PIX_FMT_JPEG) { 
		...
		struct v4l2_buffer *buf)
{
  ...
  out:
	/*
	 * FIXME!!!
	 * get mess jpeg image. So
	 * wait for CCIC DMA to DDR really done ?
	 */
...
static int ccic_v4l_open(struct file *filp)
{
	...
	int ret = 0;
	/*
	 * in case of LCD is enabled after CCIC
	 * FIXME !!! add it to clock framework other than
	 * hardcode
	 */
	__raw_writel(0x3f, APMU_LCD_CLK_RES_CTRL);
...
static u8 active_flag_restore = 0;
static void ccic_sleep_early_suspend(struct early_suspend *h)
{
	struct ccic_camera *cam = container_of(h, struct ccic_camera, ccic_early_suspend);
	active_flag_restore = cam->active_flag;
	cam->active_flag = 0;
	wake_up_all(&cam->iowait);
}
...
{
	struct ccic_camera *cam = container_of(h, struct ccic_camera, ccic_early_suspend);
	cam->active_flag = cam->active_flag;
	/*
	 * in case of LCD is enabled after CCIC
	 * FIXME !!! add it to clock framework other than
	 * hardcode
...
static int pxa910_camera_probe(struct platform_device *pdev)
{
	...
	/* do not need lock mutex in probe for we bring up sensor in late_init */
	...
	cam->rubbish_buf_virt = (void *)__get_free_pages(GFP_KERNEL, get_order(dma_buf_size));

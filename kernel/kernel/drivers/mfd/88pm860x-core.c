				       ...
				       struct pm860x_platform_data *pdata)
{
	struct i2c_client *i2c = (chip->id == CHIP_PM8607) ? chip->client \
				: chip->companion;
	int use_gpadc = 0, data, ret = -EINVAL;
				       ...
				       struct pm860x_platform_data *pdata)
{
	...
	ret = pm860x_set_bits(i2c, PM8607_SLEEP_MODE2, 3 << 4, 3 << 4);
	if (ret < 0) {
		dev_err(chip->dev, "Falied to set VLDO4 as active in sleep mode\n");
		goto out;
	}
	...
	status_val = pm860x_reg_read(i2c, PM8607_STATUS_2);
       if(status_val&PM8607_CHG_STATUS)
       	{
       	 printk("device_8607_init:uart_switch_usb\n");
       	 uart_switch_usb();
       	}
	   else
	   	{
	   	 printk("device_8607_init:usb_switch_uart\n");
	   	 usb_switch_uart();
	   	}
